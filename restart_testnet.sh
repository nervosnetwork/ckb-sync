...
if [[ $# -gt 1 || ( $# -eq 1 && $1 != 0 ) ]]; then
    echo "用法: bash restart_testnet.sh [0]" >&2
    exit 1
fi
CMD=(./ckb run)
if [[ ${1:-} == 0 ]]; then
...
NODE_DIRS=(testnet_ckb_*_x86_64-unknown-linux-gnu)
if [[ ${#NODE_DIRS[@]} -ne 1 || ! -d ${NODE_DIRS[0]} ]]; then
    echo "需要且只能有一个 testnet 节点目录" >&2
    exit 1
fi
cd -- "${NODE_DIRS[0]}"
./ckb --version

PIDS=$(sudo lsof -t -a -iTCP:8124 -sTCP:LISTEN || true)
for pid in $PIDS; do
    echo "停止 CKB PID=$pid"
    sudo kill "$pid"
done
