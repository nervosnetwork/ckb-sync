...
Mode `3`/`4` uses flag `1`, writes `result_*.log`, and periodically restarts the existing node while `is_exec=0`. Windows active modes are controlled by `mode_sequence.txt`; the default `1,2` runs mainnet, sends a report, then runs testnet and repeats without periodic restarts. Change it to `1,2,3,4` to restore the four-mode cycle.
